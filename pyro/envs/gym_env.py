...
class GymEnv(Environment):
    ...
    def __init__(self, env, is_image=False, max_episode_length=None):
        ...
        if isinstance(self._env.action_space, akro.Space):
            self._action_space = self._env.action_space
        else:
            self._action_space = akro.from_gym(self._env.action_space)
        if isinstance(self._env.observation_space, akro.Space):
    ...
    def reset(self, **kwargs):
        ...
        """
        first_obs = self._env.reset(**kwargs)
        self._step_cnt = 0
        self._env_info = None
    ...
    def step(self, action):
        ...
        if not self.spec.observation_space.contains(observation[0]):
