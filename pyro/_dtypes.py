...
class EpisodeBatch(TimeStepBatch):
    ...
    def __init__(self, env_spec, episode_infos, observations,
                 ...
                 env_infos, agent_infos, step_types, lengths):  # noqa: D102
        # lengths
        if len(lengths.shape) != 1:
            raise ValueError(
                f'lengths has shape {lengths.shape} but must be a tensor of '
                f'shape (N,)')
        ...
        if not torch.is_tensor(last_observations):
            raise ValueError(
                f'last_observations is not a pytorch tensor')
        ...
        object.__setattr__(self, 'last_masks', last_masks)
    ...
    def concatenate(cls, *batches):
        ...
        episode_infos = {
            k: torch.cat([b.episode_infos_by_episode[k] for b in batches])
        ...
        return cls(
            ...
            masks=torch.cat([batch.masks for batch in batches]),
            last_masks=torch.cat([batch.last_masks for batch in batches]),
            actions=torch.cat([batch.actions for batch in batches]),
            rewards=torch.cat([batch.rewards for batch in batches]),
            env_infos=env_infos,
            agent_infos=agent_infos,
            step_types=torch.cat([batch.step_types for batch in batches]),
            lengths=torch.cat([batch.lengths for batch in batches]))
    ...
    def split(self):
        ...
        for i, (start, stop) in enumerate(self._episode_ranges()):
            ...
            eps = EpisodeBatch(
                ...
                lengths=torch.as_tensor([self.lengths[i]]))
            episodes.append(eps)
